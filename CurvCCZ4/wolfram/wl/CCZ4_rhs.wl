...
SetEQN[trbDphibDphi[], invgamb[k, l] dphi[-k] dphi[-l]];
...
SetEQN[Rb[i_, j_], -(1/2) invgamb[k, l] hDhDgamb[-k, -l, i, j] + 1/2 (gamb[-k, i] hDLt[j, k] + gamb[-k, j] hDLt[i, k]) + 1/2 trDGam[k] (DGamDDD[i, j, -k] + DGamDDD[j, i, -k]) + invgamb[k, l] (1/2 (DGam[m, -k, i] DGamDDD[j, -m, -l] + DGam[m, -k, j] DGamDDD[i, -m, -l]) + DGam[m, i, -k] DGamDDD[-m, j, -l]) + RZ[i, j]];
...
SetEQN[dtTheta[], LbetaTheta[] + 1/2 alpha[] (psim4[] (trRb[] - 8 trbDbDphi[] - 8 trbDphibDphi[]) + 2/3 trK[] trK[] - 2 Theta[] trK[] - exAb[-i, -j] exAbUU[i, j]) - ZU[k] dalpha[-k] - ckappa1 (2 + ckappa2) Theta[] - 8 cpi alpha[] rho[]];
